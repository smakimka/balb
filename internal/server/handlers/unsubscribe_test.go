...
func TestUnsubscribe(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "subscription doesn't exists",
			method:      http.MethodPost,
			contentType: "application/json",
			body: model.SubscriptionData{
				Front:         model.TelegramFront,
			...
			mock: mock{
				expect:    true,
				returnErr: storage.ErrSubscriptionNotFound,
			},
			want: want{
				contentType: "application/json",
				code:        http.StatusNotFound,
				body:        model.Response{Msg: "subscription not found"},
			},
		},
		{
	...
	ts := httptest.NewServer(getTestUnsubscribeRouter(m))

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if test.mock.expect {
				m.EXPECT().Unsubscribe(gomock.Any(), gomock.Eq(&test.body)).Times(1).Return(test.mock.returnErr)
			} else {
				m.EXPECT().Unsubscribe(gomock.Any(), gomock.Any()).Times(0)
			}
...
func getTestUnsubscribeRouter(s storage.Storage) chi.Router {
	unsubscribeHandler := handlers.NewUnsubscribeHandler(s)

	r := chi.NewRouter()
	r.Post("/", unsubscribeHandler.ServeHTTP)
